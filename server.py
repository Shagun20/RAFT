...
class Node():
    ...
    def start_leader(self):
        ...
        self.nextIndex = [len(self.logs)+1] * len(self.conns)
        self.matchIndex = [0]*len(self.conns)
        self.cur_leader_id = self.id
        no_op_entry = ['NO-OP', self.get_metadata('term')]
    ...
    def get_val(self, var) -> tuple:
        # return format is error, value. -1,-1 if there is an error  
        self.logs = self.get_logs()

        for idx in range(len(self.logs)-1, -1, -1):
            command = self.logs[idx].split(' ')
            if command[1] == var:
                return (0, command[2])
    ...
    def send_entries(self, peer):
        ...
        if self.state.lower() == 'leader':
            ...
            append_entries_req.leaderTerm = self.get_metadata('term')
            append_entries_req.leaderId = self.id
            # min val of prevLogIndex is -1
            append_entries_req.prevLogIndex = self.nextIndex[peer] - 1 if self.nextIndex[peer] > 0 else -1
            #if prevLogIndex is -1 then setting prevLogTerm to 0
            append_entries_req.prevLogTerm = self.logs[append_entries_req.prevLogIndex][1] if append_entries_req.prevLogIndex >= 0 and len(self.logs)>0 else 0
            append_entries_req.leaderCommit = self.commitIndex
            append_entries_req.entries =entries
            ...
            if self.nextIndex[peer] <= len(self.logs):
                ...
                append_entries_req.entries = [self.logs[self.nextIndex[peer] - 1]]
            else:
                # If no new entries, sending heartbeat with empty entries
                append_entries_req.entries.extend([])
            ...
            try:
                ...
                if response.term > self.get_metadata('term'):
                    ...
                    self.become_follower(response.term)
                    print(f'{self.idD} Stepping down')
                    ...
                    if response.success :
                        ...
                        self.nextIndex[peer] = max(self.nextIndex[peer] - 1, 1)
                        # check if -1 or 0
                        self.matchIndex[peer] = self.nextIndex[peer] - 1 if self.nextIndex[peer] > 0 else 0
    ...
    def heartbeat_setter(self):
        ...
        for peer_len in self.matchIndex:
            if peer_len >= self.commitIndex:
                supportNodes += 1

        if supportNodes > len(self.conns)//2:
            self.commitIndex += 1
            self.reset_lease_timer()
            print(f"Leader {self.id}'s lease timer has reset.")
        else:
            print(f' ACKS recvd: {supportNodes}')
            print(f"Leader {self.id} lease renewal failed.")
            if self.get_lease_duration() ==0:
                print(f"Leader {self.id} lease renewal failed, lease has expired. Stepping down.")
                self.add_to_dump(f"Leader {self.id} lease renewal failed, lease has expired. Stepping down.")
                self.become_follower(self.get_metadata('term'))


        while self.lastApplied < self.commitIndex :
            
            command_parts = self.logs[self.lastApplied][0].split()
            # cmd_parts = self.logs[self.lastApplied]
            # print(cmd_parts)
            # appending no-op to committed logs
            if(command_parts[0] == 'NO-OP'):
                no_op_message = 'NO-OP'
                no_op_term = self.logs[self.lastApplied][1]
                self.lastApplied += 1
                self.committed_logs[no_op_message]= [no_op_term]
                self.write_to_committed_logs()
                print(f"Node {self.id} (leader) committed the entry {[no_op_message, no_op_term]} to the state machine.")
                self.add_to_dump(f"Node {self.id} (leader) committed the entry {[no_op_message, no_op_term]} to the state machine.")
            # appending other entries committed logs
            else:
                key =command_parts[1]
                value = command_parts[2]
                self.committed_entries[key] = value
                self.lastApplied += 1
                self.committed_logs[key]=[value, self.logs[self.lastApplied][1]]
                self.write_to_committed_logs()
                print(f"Node {self.id} (leader) committed the entry {[command_parts[0], key, value, self.logs[self.lastApplied][1]]} to the state machine.")
                self.add_to_dump(f"Node {self.id} (leader) committed the entry {[command_parts[0], key, value, self.logs[self.lastApplied][1]]} to the state machine.")
...
class ServerHandler(pb2_grpc.RaftServicer, Node):
    ...
    def AppendEntries(self, request):
        ...
        if self.get_metadata('term') <= term:
            ...
            self.success = False
            # print(f'Node {self.id} rejected AppendEntries RPC from Leader {leader_id}')
        ...
        if self.commitIndex < leaderCommit:
            ...
            self.commitIndex = min(len(self.logs) - 1, leaderCommit)
        ...
        while self.lastApplied < self.commitIndex:
            ...
            if command_parts[0] == 'NO-OP':
                ...
                self.committed_logs[no_op_message] = [no_op_term]
                self.write_to_committed_logs()
                print(f"Node {self.id} (follower) committed the entry {[no_op_message, no_op_term]} to the state machine.")
                self.add_to_dump(f"Node {self.id} (follower) committed the entry {[no_op_message, no_op_term]} to the state machine.")
            else:
                ...
                self.committed_logs[key] = [value, self.logs[self.lastApplied][1]]
                self.write_to_committed_logs()
                print(f"Node {self.id} (follower) committed the entry {[command, key, value, self.logs[self.lastApplied][1]]} to the state machine.")
                self.add_to_dump(f"Node {self.id} (follower) committed the entry {[command, key, value, self.logs[self.lastApplied][1]]} to the state machine.")

        response = {
            'term': self.get_metadata('term'),
            'success': self.success
        }
        return pb2.AppendEntriesResponse(**response)
    ...
    def ServeClient(self, request, context):
        ...
        if request.type == 'get':
            ...
            error, value = self.get_val(request.variable)
            if error:
                return response
            ...
            if not self.has_lease: #check if still leader
                return response
            resp = self.set_val(request.variable, request.value)
